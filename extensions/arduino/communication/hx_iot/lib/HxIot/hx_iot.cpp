...
void Hx_Iot::wifiConnect(String account, String password){
...
boolean Hx_Iot::connected(){
    return currentMqttStatus == iotStatusProtocol::MqttConnected && client.connected();
}

String Hx_Iot::getWiFiLocalIP()
{
    return WiFi.localIP().toString();
}

String Hx_Iot::getWiFiNetmask()
{
    return WiFi.subnetMask().toString();
}

String Hx_Iot::getGateway()
{
    return WiFi.gatewayIP().toString();
}

String Hx_Iot::getDNS()
{
    return WiFi.dnsIP().toString();
}

boolean Hx_Iot::setSoftAP(String ssid, String pass)
{
    this->account = ssid;
    this->password = pass;
...
void Hx_Iot::disconnect(){
    client.disconnect();
}

void Hx_Iot::publish(Topicnum topic_num, const String& message)
{
...
void Hx_Iot::publish(Topicnum topic_num, double f, int precision)
{
    String str;
    str = String(f,5);
    //Serial.println(str);
...
void Hx_Iot::publish(Topicnum topic_num, int64_t i)
{
    char buffer[34];
    memset(buffer, 0, 34);
    itoa(i, buffer, 10);
...
/*Use haoxue cloud platform*/
void Hx_Iot::init(String Server,
                            String ProductID, String DeviceName,
                            String DeviceSecret,
                            uint16_t port=1883)
{
    this->_MQTTSERVER    = Server;
    this->_ProductID     = ProductID;
    this->_DeviceName    = DeviceName;
    this->_DeviceSecret  = DeviceSecret;
    this->_port  = port;
    this->_UseServer     = HAOXUENET;
...
void Hx_Iot::init(String OneNetServer,
...
void Hx_Iot::init(String AliyunServer, String AliProductKey, 
...
void Hx_Iot::setIotDebugCallback(const DebugMsgHandleCb handleCb){
    iot_debugMsgHandleCb = handleCb;
}

void Hx_Iot::setMqttCallback(const MsgHandleCb handles[]){
    for(int i = 0; i < MAXTOPICNUMBER; i++){
        if(handles[i]){
...
void Hx_Iot::setMqttCallback(Topicnum topic_num, const String delimiters, const HandleCb handles){
...
void Hx_Iot::setConfig(){
    if(this->_UseServer == ONENET){
        ...
        strcpy(this->_password,tempPass.c_str());
    } else if(his->_UseServer == HAOXUENET) {
        String tempSERVER = this->_MQTTSERVER;
        uint8_t len = tempSERVER.length();
        uint16_t timestamp = 1994;
        if(this->_mqttServer == NULL){
            this->_mqttServer = (char *) malloc(len);
        }
        strcpy(this->_mqttServer,tempSERVER.c_str());
        ...
        String tempName = (this->_DeviceName + "&" + this->_ProductID);
        len = tempName.length();
        this->_username = (char * )malloc(len);
        strcpy(this->_username,tempName.c_str());
